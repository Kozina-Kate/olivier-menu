...
`GET /api/v1/recipes/matches/?ingredients=1,2,3` подбирает опубликованные
рецепты по ID продуктов из этого каталога. Ответ содержит `count` и `results`:
для каждого рецепта — полные данные блюда, `can_cook`, `match_percent`,
совпавшие продукты и недостающие обязательные. Необязательные продукты не
...
Выбор сохраняется в рамках вкладки при переходе в рецепт и обратно. Если API
недоступен, экран продуктов показывает ошибку и кнопку повтора, а `/menu`
явно помечает свой резервный демонстрационный рецепт.
